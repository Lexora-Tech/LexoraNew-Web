...
$search = '';
$sql = "SELECT q.*, c.name as customer_name FROM quotations q LEFT JOIN customers c ON q.customer_id = c.id ORDER BY q.created_at DESC";
if (isset($_GET['search']) && !empty($_GET['search'])) {
    $search = mysqli_real_escape_string($conn, $_GET['search']);
    $sql = "SELECT q.*, c.name as customer_name FROM quotations q LEFT JOIN customers c ON q.customer_id = c.id WHERE q.quotation_number LIKE '%$search%' OR c.name LIKE '%$search%' ORDER BY q.created_at DESC";
}
...
function getStatusBadge($s)
{
    ...
    $st = $c[$s] ?? 'background:rgba(107,114,128,0.1);color:#6b7280;';
...
<head>
    ...
    <title>Quotations | Lexora Admin</title>
...
<body>
        ...
        <div class="main-content">
            <header>
                ...
                <div class="header-right">
                    <form class="search-box" method="GET"><button type="submit" class="search-button"><i
                            ...
                            placeholder="Search quotations..." value="<?= htmlspecialchars($search)?>"></form>
            ...
            <div class="content-wrapper">
                <div class="section-header">
                    <h3>
                        <?= empty($search) ? 'All Quotations' : "Results for '" . htmlspecialchars($search) . "'"?>
                    </h3><a href="quotation_form.php" class="btn-add"><i class="fas fa-plus"></i> New Quotation</a>
                </div>
                    ...
                    <div class="table-responsive">
                        <table>
                            ...
                            <tbody>
                                <?php if ($count == 0): ?>
                                <tr>
                                    <td colspan="7" style="text-align:center;color:var(--text-muted);padding:40px;"><i
                                            class="fas fa-file-invoice"
                                            style="font-size:30px;display:block;margin-bottom:10px;"></i>No quotations
                                        yet.</td>
                                </tr>
                                <?php
else:
    while ($row = mysqli_fetch_assoc($result)): ?>
                                <tr>
                                    ...
                                    <td style="color:var(--text-muted);font-size:13px;">
                                        <?= $row['valid_until'] ? date("M d, Y", strtotime($row['valid_until'])) : '-'?>
                                    </td>
                                    <td><strong>
                                    ...
                                    <td>
                                        <div class="actions">
                                            <a href="quotation_view.php?id=<?= $row['id']?>" class="btn-icon"
                                                title="View"><i class="fas fa-eye"></i></a>
                                            <a href="quotation_form.php?id=<?= $row['id']?>" class="btn-icon"
                                                title="Edit"><i class="fas fa-pen"></i></a>
                                            <?php if ($row['status'] !== 'converted'): ?>
                                            <a href="convert_to_invoice.php?id=<?= $row['id']?>"
                                                class="btn-icon convert" title="Convert to Invoice"
                                                onclick="return confirm('Convert this quotation to an invoice?')"><i
                                                    class="fas fa-exchange-alt"></i></a>
                                            <?php
        endif; ?>
                                            <a href="generate_pdf.php?type=quotation&id=<?= $row['id']?>"
                                                class="btn-icon" title="Download PDF" target="_blank"><i
    ...
    <script>
        ...
        let deleteId = null;
        document.querySelectorAll('.delete').forEach(b => { b.addEventListener('click', () => { deleteId = b.getAttribute('data-id'); document.getElementById('deleteModal').style.display = 'flex'; }); });
        document.getElementById('confirmNo').addEventListener('click', () => { document.getElementById('deleteModal').style.display = 'none'; });
        document.getElementById('confirmYes').addEventListener('click', () => { window.location.href = `quotation_delete.php?id=${deleteId}`; });
...
<?php if  (isset($_GET['msg'])): ?>
            function showToast(m) { const c = document.getElementById('toastContainer'), t = document.createElement('div'); t.className = 'toast'; t.innerHTML = `<i class="fas fa-check-circle" style="color:#10b981"></i> ${m}`; c.appendChild(t); setTimeout(() => { t.style.opacity = '0'; setTimeout(() => t.remove(), 300); }, 3000); }
