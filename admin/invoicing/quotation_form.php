...
$editing = false;
$quotation = ['customer_id' => '', 'status' => 'draft', 'issue_date' => date('Y-m-d'), 'valid_until' => date('Y-m-d', strtotime('+30 days')), 'payment_terms' => '', 'notes' => ''];
$items = [];

if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $q = mysqli_query($conn, "SELECT * FROM quotations WHERE id=$id");
...
<head>
    ...
    <title>
        <?= $editing ? 'Edit' : 'New'?> Quotation | Lexora Admin
    </title>
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            ...
            <div class="content-wrapper">
                <form action="quotation_save.php" method="POST" id="quotationForm" class="form-container">
                    <div class="form-title"><i class="fas fa-file-invoice"
                            style="color:var(--primary);margin-right:10px;"></i>
                        <?= $editing ? 'Edit Quotation #' . htmlspecialchars($quotation['quotation_number']) : 'Create New Quotation'?>
                    </div>
                    <?php if ($editing): ?><input type="hidden" name="id" value="<?= $quotation['id']?>">
                    <?php
endif; ?>
...
endwhile; ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="issue_date" class="form-input"
                                value="<?= $quotation['issue_date']?>" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" class="form-select">
                                <?php foreach (['draft', 'sent', 'accepted', 'rejected', 'cancelled'] as $s): ?>
...
endforeach; ?>
                        ...
                        <div class="form-group full">
                            ...
                            <textarea name="notes" class="form-textarea"
                                placeholder="Additional notes for the client..."><?= htmlspecialchars($quotation['notes'] ?? '')?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="items-section">
                        <div class="items-header">
                            ...
                            <button type="button" class="btn-add-item" onclick="addItem()"><i class="fas fa-plus"></i>
                                Add Item</button>
                        </div>
                    ...
                    <div class="btn-row">
                        <a href="quotations.php" class="btn-secondary">Cancel</a>
                        <button type="submit" class="btn-primary"><i class="fas fa-save"></i>
                            <?= $editing ? 'Update' : 'Save'?> Quotation
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        function addItem(desc = '', qty = 1, price = 0, tax = 0, discount = 0) {
            ...
            tr.innerHTML = `
        <td><input type="text" name="items[${itemIndex}][description]" value="${desc}" placeholder="Service or product description" required></td>
        <td class="num-col"><input type="number" name="items[${itemIndex}][quantity]" value="${qty}" min="0.01" step="0.01" class="calc" placeholder="1"></td>
        <td class="num-col"><input type="number" name="items[${itemIndex}][unit_price]" value="${price}" min="0" step="0.01" class="calc" placeholder="0.00"></td>
        <td class="num-col"><input type="number" name="items[${itemIndex}][tax_rate]" value="${tax}" min="0" max="100" step="0.01" class="calc" placeholder="0"></td>
        <td class="num-col"><input type="number" name="items[${itemIndex}][discount]" value="${discount}" min="0" step="0.01" class="calc" placeholder="0.00"></td>
    ...
    `;
            ...
            itemIndex++;
            tr.querySelectorAll('.calc').forEach(i => i.addEventListener('input', recalculate));
            recalculate();
        }
        ...
        function recalculate() {
            ...
            document.querySelectorAll('#itemsBody tr').forEach((tr, idx) => {
                ...
                const taxRate = parseFloat(tr.querySelector('[name*="tax_rate"]').value) || 0;
