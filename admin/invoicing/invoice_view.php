...
// Get payments
$payments = [];
$pay_q = mysqli_query($conn, "SELECT * FROM payments WHERE invoice_id=$id ORDER BY payment_date DESC");
...
<head>
    ...
    <title>Invoice
        <?= htmlspecialchars($inv['invoice_number'])?> | Lexora Admin
    </title>
                ...
                <div class="doc-card">
                    ...
                    <div class="info-grid">
                        <div class="info-block">
                            ...
                            <p>
                                <?= htmlspecialchars($inv['customer_company'] ?? '')?>
                            </p>
                            <p>
                                <?= htmlspecialchars(($inv['customer_address'] ?? '') . ($inv['customer_city'] ? ', ' . $inv['customer_city'] : '') . ($inv['customer_country'] ? ', ' . $inv['customer_country'] : ''))?>
                            </p>
                        </div>
                        <div class="info-block" style="text-align:right;">
                            <h4>Invoice Info</h4>
                            <p><strong>Issue Date:</strong>
                                <?= date("M d, Y", strtotime($inv['issue_date']))?>
                            </p>
                            <p><strong>Due Date:</strong>
                                <?= $inv['due_date'] ? date("M d, Y", strtotime($inv['due_date'])) : '-'?>
                            </p>
                            <?php if ($inv['payment_terms']): ?>
                            <p><strong>Terms:</strong>
                                <?= htmlspecialchars($inv['payment_terms'])?>
                            </p>
                            <?php
endif; ?>
                        </div>
                    ...
                    <div class="totals">
                        <div class="totals-box">
                            ...
                            <div class="total-row"><span>Tax</span><span>
                                    <?= format_currency($inv['tax_amount'])?>
                                </span></div>
                            <div class="total-row"><span>Discount</span><span>-
                                    <?= format_currency($inv['discount_amount'])?>
                                </span></div>
                            <div class="total-row grand"><span>Grand Total</span><span>
                                    <?= format_currency($inv['grand_total'])?>
                                </span></div>
                            <div class="total-row paid"><span>Amount Paid</span><span>
                                    <?= format_currency($inv['amount_paid'])?>
                                </span></div>
                            <div class="total-row balance"><span>Balance Due</span><span>
                                    <?= format_currency($inv['grand_total'] - $inv['amount_paid'])?>
                                </span></div>
                        </div>
                    </div>
                    <?php if ($inv['notes']): ?>
