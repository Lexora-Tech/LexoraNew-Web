...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql = "SELECT * FROM customers WHERE name LIKE '%$search%' OR email LIKE '%$search%' OR company LIKE '%$search%' ORDER BY created_at DESC";
}
...
<head>
    ...
    <title>Customers | Lexora Admin</title>
...
<body>
        ...
        <div class="main-content">
            <header>
                ...
                <div class="header-right">
                    <form class="search-box" method="GET" action=""><button type="submit" class="search-button"><i
                                class="fas fa-search"></i></button><input type="text" name="search"
                            placeholder="Search customers..." value="<?= htmlspecialchars($search)?>"></form>
            ...
            <div class="content-wrapper">
                <div class="section-header">
                    <h3>
                        <?= empty($search) ? 'All Customers' : "Results for '" . htmlspecialchars($search) . "'"?>
                    </h3>
                    <a href="customer_form.php" class="btn-add"><i class="fas fa-plus"></i> Add Customer</a>
                </div>
                    ...
                    <div class="table-responsive">
                        <table>
                            ...
                            <tbody>
                                <?php if ($count == 0): ?>
                                <tr>
                                    <td colspan="5" style="text-align:center;color:var(--text-muted);padding:40px;"><i
                                            class="fas fa-users"
                                            style="font-size:30px;display:block;margin-bottom:10px;"></i>No customers
                                        found.</td>
                                </tr>
                                <?php
else:
    while ($row = mysqli_fetch_assoc($result)): ?>
                                <tr>
                                    ...
                                    <td>
                                        <div class="actions">
                                            <a href="customer_form.php?id=<?= $row['id']?>" class="btn-icon"
    ...
    <div class="modal" id="deleteModal">
        <div class="modal-content">
            ...
            <p style="color:var(--text-muted);margin-bottom:20px;">This will permanently remove this customer. Continue?
            </p>
    ...
    <script>
        ...
        document.querySelectorAll('.delete').forEach(btn => {
            btn.addEventListener('click', () => { deleteId = btn.getAttribute('data-id'); document.getElementById('deleteModal').style.display = 'flex'; });
        });
        document.getElementById('confirmNo').addEventListener('click', () => { document.getElementById('deleteModal').style.display = 'none'; });
        document.getElementById('confirmYes').addEventListener('click', () => { window.location.href = `customer_delete.php?id=${deleteId}`; });
...
<?php if  (isset($_GET['msg'])): ?>
            function showToast(m) { const c = document.getElementById('toastContainer'), t = document.createElement('div'); t.className = 'toast'; t.innerHTML = `<i class="fas fa-check-circle" style="color:var(--success)"></i> ${m}`; c.appendChild(t); setTimeout(() => { t.style.opacity = '0'; setTimeout(() => t.remove(), 300); }, 3000); }
