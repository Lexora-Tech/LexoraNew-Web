<!DOCTYPE html>
<html lang="en">
<head>
    <?php
    $pageTitle = "Lexora Tech | POS Software Development";
    $pageDesc = "Efficient and customizable POS solutions. Streamline your retail or restaurant operations with our tailored point-of-sale software.";
    include "includes/head.php";
    ?>
</head>
<body>

    <!-- wrapper -->
    <div id="smooth-wrapper" class="mil-page-wrapper">

        <!-- cursor -->
        <div class="mil-cursor-follower"></div>

        <!-- scroll progress -->
        <div class="mil-progress-track">
            <div class="mil-progress"></div>
        </div>

        <!-- fixed elements -->
        <?php include "header.php"; ?>

        <!-- page transition -->
        <div class="mil-transition-fade" id="swup">
            <div class="mil-transition-frame">

                <!-- content -->
                <div id="smooth-content" class="mil-content">

                    <!-- hero -->
                    <div class="mil-hero-1 mil-sm-hero mil-stl mil-up" id="top">
                        <div class="mil-overlay"></div>
                        <div class="container mil-hero-main mil-relative mil-aic">
                            <div class="mil-hero-text mil-scale-img" data-value-1="1.3" data-value-2="0.95">
                                <div class="mil-text-pad"></div>
                                <ul class="mil-breadcrumbs mil-mb60 mil-c-gone">
                                    <li><a href="index.php">Home</a></li>
                                    <li><a href="#.">Services</a></li>
                                </ul>
                                <h1 class="mil-display2 mil-rubber">POS Development</h1>
                            </div>
                        </div>
                    </div>
                    <!-- hero end -->

                    <!-- title -->
                    <div class="container mil-relative">
                        <div class="row mil-jcc">
                            <div class="col-md-10">
                                <h1 class="mil-head2 mil-tac mil-mb160 mil-up">
                                    <span class="mil-a2">POS Development</span> isn’t just about transactions —
                                    it’s about <span class="mil-a2">empowering businesses</span> with intelligent, reliable,
                                    and seamless point-of-sale experiences.
                                </h1>
                            </div>
                        </div>
                    </div>
                    <!-- title end -->

                    <!-- iconboxes -->
                    <div class="mil-p-0-100" id="scroll">
                        <div class="container">
                            <div class="row mil-jcc">
                                <div class="col-sm-8 col-lg-4">
                                    <div class="mil-iconbox mil-tac mil-mb60">
                                        <img src="img/logo/1.png" alt="icon" class="mil-mb30 mil-up">
                                        <h4 class="mil-head4 mil-mb30 mil-up">Custom POS Solutions</h4>
                                        <p class="mil-text-md mil-shortened mil-up">
                                            We design POS systems tailored to your operations — from retail and restaurants to salons and enterprises — integrating every feature your business needs to run smoothly.
                                        </p>
                                    </div>
                                </div>
                                <div class="col-sm-8 col-lg-4">
                                    <div class="mil-iconbox mil-mb60">
                                        <img src="img/logo/2.png" alt="icon" class="mil-mb30 mil-up">
                                        <h4 class="mil-head4 mil-mb30 mil-up">Seamless Integration</h4>
                                        <p class="mil-text-md mil-shortened mil-up">
                                            Connect your POS with inventory, CRM, accounting, and online stores for real-time synchronization across every sales channel.
                                        </p>
                                    </div>
                                </div>
                                <div class="col-sm-8 col-lg-4">
                                    <div class="mil-iconbox mil-tac mil-mb60">
                                        <img src="img/logo/3.png" alt="icon" class="mil-mb30 mil-up">
                                        <h4 class="mil-head4 mil-mb30 mil-up">Secure & Scalable</h4>
                                        <p class="mil-text-md mil-shortened mil-up">
                                            Built with top-tier security and scalable architecture, our POS systems grow with your business — ensuring smooth operations from one store to hundreds.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- iconboxes end -->

                    <!-- service -->
                    <div class="mil-p-0-100 mil-relative">
                        <div class="container">
                            <div class="row mil-jcb">
                                <div class="col-md-5">
                                    <p class="mil-text-md mil-mb60 mil-up">
                                        At Lexora Tech, we build POS systems that go beyond transactions —
                                        they simplify operations, improve customer experience, and deliver insights that drive business growth.
                                    </p>
                                    <div class="mil-team-quote mil-mb60 mil-up">
                                        <div class="mil-portrait">
                                            <img src="img/team/companyLogo.jpg" alt="Lexora Tech logo">
                                        </div>
                                        <p class="mil-text-md mil-m1">
                                            <span class="mil-bold">Smart. Secure. Scalable.</span><br>
                                            POS systems built to <span class="mil-bold">empower your business</span>.
                                        </p>
                                    </div>
                                    <div class="mil-counter-item mil-mb160 mil-up">
                                        <h4 class="mil-up">100<span class="mil-a2">+</span></h4>
                                        <div class="mil-counter-text">
                                            <h5 class="mil-head4 mil-m1 mil-up">Successful POS Deployments</h5>
                                        </div>
                                    </div>
                                </div>

                                <div class="col-md-6">
                                    <h3 class="mil-head4 mil-mb30 mil-up"><span class="mil-a2">01.</span>&nbsp; Requirements & Analysis</h3>
                                    <p class="mil-text-md mil-mb60 mil-up">
                                        We start by understanding your workflows — from billing to inventory and reporting — ensuring the POS system perfectly fits your operational model.
                                    </p>

                                    <h3 class="mil-head4 mil-mb30 mil-up"><span class="mil-a2">02.</span>&nbsp; UI/UX Design</h3>
                                    <p class="mil-text-md mil-mb60 mil-up">
                                        Our design experts craft intuitive interfaces that make transactions faster and simpler, reducing staff training time and increasing efficiency.
                                    </p>

                                    <h3 class="mil-head4 mil-mb30 mil-up"><span class="mil-a2">03.</span>&nbsp; Development & Integration</h3>
                                    <p class="mil-text-md mil-mb60 mil-up">
                                        Using robust technologies like Laravel, React, Node.js, and Flutter, we develop cloud-based and offline-compatible POS systems with integrated analytics, stock tracking, and multi-branch support.
                                    </p>

                                    <h3 class="mil-head4 mil-mb30 mil-up"><span class="mil-a2">04.</span>&nbsp; Testing, Deployment & Maintenance</h3>
                                    <p class="mil-text-md mil-mb30 mil-up">
                                        We ensure smooth operations through rigorous testing and seamless deployment. Post-launch, we provide continuous updates, performance monitoring, and dedicated technical support.
                                    </p>

                                    <p class="mil-text-md mil-mb60 mil-up">
                                        From single-outlet solutions to enterprise-grade POS networks, Lexora Tech delivers systems that optimize sales, automate reports, and provide real-time business intelligence.
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- service end -->

                    <!-- call to action -->
                    <div class="container">
                        <div class="mil-half-container mil-stl mil-up">
                            <div class="mil-text-box mil-g-m4 mil-p-160-160">
                                <p class="mil-stylized mil-m2 mil-mb60 mil-up">Portfolio</p>
                                <h2 class="mil-display3 mil-rubber mil-mb60 mil-m1 mil-up">Explore our <span class="mil-a1">projects</span> and get inspired</h2>
                                <div class="mil-up"><a href="portfolio.php" class="mil-btn mil-btn-border mil-m1 mil-c-gone">View projects</a></div>
                            </div>
                            <div class="mil-image-box">
                                <div class="mil-image-frame">
                                    <img src="img/works/1/img1.jpg" alt="img" class="mil-scale-img" data-value-1="1.20" data-value-2="1">
                                </div>
                            </div>
                        </div>
                        <div class="row mil-aic mil-jcb mil-no-g">
                            <div class="col-lg-6">
                                <div class="mil-button-pad mil-a1 mil-jst" style="display: block"></div>
                            </div>
                            <div class="col-lg-6 mil-992-gone">
                                <div class="mil-text-pad">
                                    <p class="mil-stylized mil-up">Designing a Better World Today</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- call to action end -->

                    <!-- footer -->
                    <?php include "footer.php"; ?>
                    <!-- footer end -->

                </div>
                <!-- content end -->

            </div>
        </div>
        <!-- page transition end -->

    </div>
    <!-- wrapper end -->

    <!-- scripts -->
    <script src="js/plugins/swup.min.js"></script>
    <script src="js/plugins/gsap.min.js"></script>
    <script src="js/plugins/ScrollSmoother.min.js"></script>
    <script src="js/plugins/ScrollTrigger.min.js"></script>
    <script src="js/plugins/ScrollTo.min.js"></script>
    <script src="js/plugins/swiper.min.js"></script>
    <script src="js/plugins/parallax.js"></script>
    <script src="js/main.js"></script>


</body>

</html>